...
\begin{example}\label{eg:cotorsion-to-lifting}
	定义范畴向函子范畴的嵌入 $\mathfrak t : X \mapsto (0 \to X)$ 与 $\mathfrak s : X \mapsto (X \to 0)$. 给定余挠对 $(\mathcal{U},\mathcal{V})$, 取 $\mathfrak s (\mathcal{U})$ 余生成的弱垂直对 $(^\pitchfork(\mathfrak s (\mathcal{U})^\pitchfork), \mathcal{U}^\pitchfork)$. 此时有如下结论.
	\begin{enumerate}
		\item 给定 conflation $V \overset i \rightarrowtail B \overset p \twoheadrightarrow C$, $V \in \mathcal{V}$, 则 $p \in \mathcal{U}^\pitchfork$. 由长正合列\Cref{eq:ext-tri-6-term} 容易验证.
		\item 若 $\mathfrak t (X) \in {}^\pitchfork(\mathfrak s (\mathcal{U})^\pitchfork)$, 则 $X \in \mathcal{U}$.
		\begin{proof}
...
\begin{remark}
	容易发现, 所有形如 $(Y \to 0)$ 的态射属于 $\mathfrak s (\mathcal{U})^\pitchfork$, 这一态射类的关键信息蕴含在 inflation 的 $\mathrm{coCone}$-项中. 将余生成关系改作生成关系, 则有对偶的结论.
\end{remark}
