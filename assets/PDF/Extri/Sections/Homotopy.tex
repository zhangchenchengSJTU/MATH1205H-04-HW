...
\begin{theorem}
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            \item (可裂满). 由 $(0 \to A) \pitchfork p$, 得 $(-, p)_{\mathcal{C} \cap \mathcal{F}}$ 是满自然变换. 由\Cref{lem:mono-epi-split} 知 $p$ 是可裂满, 故 $\pi (-, p)$ 可裂满.
...
\begin{definition}\label{def:lesh-transform}
    ...
    \begin{equation}
        \ell : \mathbb E(Z,X) \to (Z, \Sigma X),\quad \eta \mapsto [a] \circ [s]^{-1}.
    \end{equation}
